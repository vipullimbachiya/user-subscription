...
import { saveProfileImage,checkAuthStatus,deleteProfileImage,logout,fetchUsers,showNotification} from '../actions/auth/auth_actions';
import {auth} from '../../config/firebase_config';
import {BootstrapTable, TableHeaderColumn, InsertButton} from 'react-bootstrap-table';
import {Navbar, Nav, NavItem, NavDropdown, MenuItem, Button, Col, Grid, Row,Modal} from 'react-bootstrap';
   ...
   /**
    ...
    randomizeFilename(name) {
        const extension = name
            .split(".")
            .pop() || "jpg";
        return (Math.random().toString(36) + '00000000000000000').slice(2, 10 + 2) + "." + extension;
    }     
   /**
    ...
    nameFormatter(cell, row) {
        const self = (row.id === auth().currentUser.uid) ? 'self':'';
        return (
                            ...
                            <div className="container">
                                ...
                                <div className="collapse navbar-collapse" id="app-navbar-collapse">
                                    <ul className="nav navbar-nav">
                                        <li><a href="#">User Subscription</a></li>
                                    </ul>
                            ...
                            <div className="col-lg-12">
                                ...
                                >
                                    <TableHeaderColumn
                                        ...
                                        dataSort={true}
                                        dataField='full_name'>Name</TableHeaderColumn>
                                    <TableHeaderColumn
                                        width="5%"
                                        dataFormat={this.imageFormatter}
                                        dataSort={true}
                                        dataField='photoURL'>Avatar</TableHeaderColumn>
                                    <TableHeaderColumn
                                        width="8%"
                                        dataFormat={this.dateFormatter}
                                        dataSort={true}
                                        dataField='createdOn'>CreatedAt</TableHeaderColumn>
                                    <TableHeaderColumn
                                        width="10%"
                                        dataFormat={this.dateFormatter}
                                        dataSort={true}
                                        dataField='lastLoggedInOn'>Last Login</TableHeaderColumn>
                                </BootstrapTable>    
                            </div>
                        </div>
                        <Modal 
...
export default connect(state => ({notifications: state.notifications,auth: state.Auth,profile_image:state.Auth.profile_image,isLoggedin: state.Auth.data}), {deleteProfileImage,saveProfileImage,showNotification,checkAuthStatus, logout,fetchUsers})(Dashboard);